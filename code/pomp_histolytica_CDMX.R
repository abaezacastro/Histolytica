...
################################################
pomp_estimation_daily<-function(pomp_obj=NA,covar_pomp,Nsim=100,parameters=NA){
...
double foi=beta_D * C_D + beta_E *(1 +  gamma * RI/(alpha + RI)) * C_E;
...
histolytica_rdpr<- Csnippet("
  ...
  double RI = RR/Rain_max;
  //dilution effect of rainfall on cyst concentration
  double v_E=v0 + v_r * RI;
  //force of infection
  double foi=(beta_D * C_D + beta_E *(1 +  gamma *  RI/(alpha + RI)) * C_E) * (dW/dt);
 //Population size
  int N=S+E+I+R;
...
histolytica_pomp <- pomp(
              ...
              rmeasure=rmeas,
              dmeasure=dmeas,
              covar=covariate_table(time=covar_pomp2$time,RR=covar_pomp2$RR,times ="time"),
...
rdd_b<-rw.sd(sigma_process=0.001,
             rho=0.0001,
             sigma_observation=0.0001,
             beta_D=0.0001,
             beta_E=0.001,
...
#Initial_paramters[["alpha"]]<-0.2
#Initial_paramters[["gamma"]]<-100
...
if(is.na(pomp_obj)){
  estpars <- setdiff(names(theta),c("delta",
                                    "Rain_max","N_0",
                                    "b","d","s","cyst_pp"))
  }
else{
...
min_par=c(sigma_process=0.01, #noise
           rho=0.03,#reporting rate
           sigma_observation=0.1, #par noise
           beta_D=2,# probability of infection associated to the concentration of cysts in households
           beta_E=10,# probability of infection associated to the concentration of cysts in the environtment
           gamma=10,
           alpha=0,
           v_r=80,#91,
           v0=0.2,
           ...
           E_0=0.01,
           I_0=0.05,
           C_E0=0.01, #concentration of pathogens in water. measure as number of cysts in a volume of water 
           C_D0=0.01)

max_par=c(sigma_process=0.02, #noise
          rho=0.05,#reporting rate
          sigma_observation=0.2, #par noise
          beta_D=10,# probability of infection associated to the concentration of cysts in households
          beta_E=30,# probability of infection associated to the concentration of cysts in the environtment
          gamma=60,
          alpha=3,
          v_r=200,#91,
          v0=0.9,
          ...
          E_0=0.04,
          I_0=0.2,
          C_E0=0.03, #concentration of pathogens in water. measure as number of cysts in a volume of water 
          C_D0=0.2)
...
as.data.frame()%>% 
  mutate(delta=rep(coef(miff2)["delta"],nsequ),
               N_0=rep(coef(miff2)["N_0"],nsequ),
               N_0=rep(coef(miff2)["N_0"],nsequ),
               cyst_pp=rep(coef(miff2)["cyst_pp"],nsequ),
               Rain_max=rep(coef(miff2)["Rain_max"],nsequ),
               b=rep(coef(miff2)["b"],nsequ),
               d=rep(coef(miff2)["d"],nsequ),
               s=rep(coef(miff2)["s"],nsequ)
               )-> pr_1
...
foreach (p=iterators::iter(pr_1,"row"),
  ...
  miff2 %>% 
    ...
    mif2(params=unlist(p),
         ...
         rw.sd=rdd_c) %>%
    mif2() -> mf
  print(c("First search",logLik(mf)))
...
foreach (p=iterators::iter(pd_3,"row"),
  ...
  miff2 %>% 
    ...
    mif2(params=unlist(p),
         ...
         rw.sd=rdd_c) %>%
    mif2() -> mf
  print(c("second search",logLik(mf)))
  pf <- replicate(10,pfilter(mf,Np=1000))  ## independent particle filters
  ll <- sapply(pf,logLik)
  ll <- logmeanexp(ll,se=TRUE)
  nfail <- sapply(pf,getElement,"nfail")  ## number of filtering failures
